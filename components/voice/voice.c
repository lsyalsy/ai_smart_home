...
bool voice_init(void)
{
    ...
    s_hangover       = 0;
    s_print_energy   = true;
    s_frame_energy_sum  = 0;
    s_frame_energy_max  = 0;
    s_frame_energy_peak = 0;
    s_frame_count       = 0;
    s_stats_last_us     = esp_timer_get_time();
    s_initialized    = true;

    ESP_LOGI(TAG, "voice init ok, VAD threshold=%lu, frame=%d samples",
             (unsigned long)s_vad_threshold, VAD_SAMPLE_COUNT);
    ESP_LOGI(TAG, "Speak to the microphone and watch energy values below.");
    return true;
}
...
vad_state_t voice_process(void)
{
    ...
    uint32_t energy = 0;
    uint32_t peak   = 0;
    voice_calc_stats(s_pcm_buffer, read_cnt, &energy, &peak);

    /* 累计每秒统计 */
    s_frame_energy_sum += energy;
    if (energy > s_frame_energy_max) {
        s_frame_energy_max = energy;
    }
    if (peak > s_frame_energy_peak) {
        s_frame_energy_peak = peak;
    }
    s_frame_count++;

    /* 每秒打印一次能量统计，方便观察麦克风是否正常工作 */
    int64_t now = esp_timer_get_time();
    if ((uint32_t)((now - s_stats_last_us) / 1000) >= 1000) {
        if (s_print_energy && s_frame_count > 0) {
            uint32_t avg = s_frame_energy_sum / s_frame_count;
            ESP_LOGI(TAG, "MIC energy avg=%-4lu max=%-4lu peak=%-5lu  [%s]",
                     (unsigned long)avg,
                     (unsigned long)s_frame_energy_max,
                     (unsigned long)s_frame_energy_peak,
                     (s_vad_state == VAD_STATE_ACTIVE) ? "HEARD SOUND" : "silent");
        }
        s_frame_energy_sum  = 0;
        s_frame_energy_max  = 0;
        s_frame_energy_peak = 0;
    ...
    if (is_active) {
        ...
        if (s_vad_state != VAD_STATE_ACTIVE) {
            ...
            ESP_LOGI(TAG, "VAD ACTIVE: energy=%lu, peak=%lu -> HEARD SOUND",
                     (unsigned long)energy, (unsigned long)peak);
        }
    } else {
        if (s_hangover > 0) {
            s_hangover--;
...
{
    s_print_energy = enable;
    ESP_LOGI(TAG, "energy print %s", enable ? "enabled" : "disabled");
}
